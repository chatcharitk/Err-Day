...
export interface ParsedSlip {
  ...
  amount:     number | null;
  /** Transfer time as shown on the slip, e.g. "14:36" — display hint only. */
  time:       string | null;
...
const TOOL_DEF: Anthropic.Tool = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      senderName: { type: ["string", "null"], description: "Sender name as printed on the slip." },
    },
    required: ["amount", "time", "bank", "senderName"],
  },
};
